          ...
          <?php 
            ...
            if(isset($_GET['TenHH']))
              $TenHH=$_GET['TenHH'];
				...
				echo "<script type='text/javascript'>location.href = '?option=shop'; </script>";
            $sql_TenHH="select*from hanghoa where status=1 and TenHH like'%".$_GET['TenHH']."%'";
            $ds_tlh=$connect->query($sql_TenHH);
             echo'<h3 class="h4 mb-0" style="font-weight: normal;">Kết quả tìm kiếm "<b>'.$TenHH.'</b>":</h3>';
          ?>
        </div>
      ...
      <div class="row">
        <?php
          ...
          $total_row;
            ...
            $total_row = $connect->query("select*from hanghoa where status=1 and TenHH like'%".$_GET['TenHH']."%'")->num_rows;
          ...
          if($pre_page==0)
           ...
           $list_page='<li class="page-item"><a class="page-link" href="?option=search&TenHH='.$TenHH.'&page='.$pre_page.'" aria-label="Previous">
                       <span aria-hidden="true">«</span></a></li>';
          if($page-1 > 1){
            $list_page.='<li class="page-item"><a class="page-link" href="?option=search&TenHH='.$TenHH.'&page=1">1</a></li>';
            if($page-1 > 2)
          ...
          for($i=$start;$i<= $end;$i++){
            if($i==$page)
              ...
              $list_page.='<li class="page-item"><a class="page-link" href="?option=search&TenHH='.$TenHH.'&page='.$i.'">'.$i.'</a></li>';
            }
          if($page+1<$total_page-1){
            $list_page.='<li class="page-item "><span class="page-link" ><i class="fa fa-ellipsis-h"></i></span></li>';
          }
          if($page+1<$total_page){
            $list_page.='<li class="page-item"><a class="page-link" href="?option=search&TenHH='.$TenHH.'&page='.$total_page.'">'.$total_page.'</a></li>';
          }
          $next_page=$page+1;
          if($next_page>$total_page)
            $list_page.=' <li class="page-item"><a class="page-link" href="" aria-label="Next"><span aria-hidden="true">»</span></a></li>';
          else
            $list_page.=' <li class="page-item"><a class="page-link" href="?option=search&TenHH='.$TenHH.'&page='.$next_page.'" aria-label="Next">
                          <span aria-hidden="true">»</span></a></li>';
          $sql_hanghoa;
          
            $sql_hanghoa="select*from hanghoa where status=1 and TenHH like'%".$_GET['TenHH']."%' limit $start_row, $row_per_page";
